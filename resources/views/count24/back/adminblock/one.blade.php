...
<table>
    ...
    <tbody>
            ...
            @if(($i % 16) == 0 || $i == 0)
                @if($info[$i]['ekr']['shared'] == "Yes" || $info[$i]['ekr']['main'] == "Yes")
                    <tr>
                        ...
                        @for ($n = 0 ; $n < 16 ; $n++)
                            <td><b>{{ number_format($info[$i+$n]['sum_fu'], 2, ',', ' ') }}<b></td>
                        @endfor
                        @for ($n = 0 ; $n < 16 ; $n++)
                            <td><font color="{{ $color }}"><b>{{ number_format($info[$i+$n]['sum_cb'], 2, ',', ' ') }}<b></td>
                        @endfor  
                    </tr>
                @else
                    <tr>
        ...
        <tr>
            ...
            <td></td>
            @for ($n = 0 ; $n < 16 ; $n++)
               <td><b>{{ number_format($fu[$n], 2, ',', ' ') }}</b></td> 
            @endfor  
            @for ($n = 0 ; $n < 16 ; $n++)
               <td><font color="{{ $color }}"><b>{{ number_format($cb[$n], 2, ',', ' ') }}</b></td>
            @endfor
        </tr>
    </tbody>
</table>
