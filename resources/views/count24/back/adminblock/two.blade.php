...
<table>
    ...
    <tbody>
            ...
            @if(($i % 19) == 0 || $i == 0)
                @if($info[$i]['ekr']['shared'] == "Yes" || $info[$i]['ekr']['main'] == "Yes")
                    <tr>
                        ...
                        @endphp     
                        <td><b>{{ number_format($info[$i]['sum_fu'] + $info[$i+1]['sum_fu'] + $info[$i+2]['sum_fu'] + $fu_adm, 2, ',', ' ') }}<b></td>
                        ...
                        <td><b>{{ number_format($fu_adm, 2, ',', ' ') }}<b></td>
                        <td><font color="{{ $color }}"><b>{{ number_format($info[$i]['sum_cb'] + $info[$i+1]['sum_cb'] + $info[$i+2]['sum_cb'] + $cb_adm, 2, ',', ' ') }}<b></td>
                        ...
                        <td><font color="{{ $color }}"><b>{{ number_format($cb_adm, 2, ',', ' ') }}<b></td>
                    </tr>
                @else
                    <tr>
                        @php 
                        ...
                        <td>{{ number_format($info[$i]['sum_fu'] + $info[$i+1]['sum_fu'] + $info[$i+2]['sum_fu'] + $fu_adm, 2, ',', ' ') }}</td>
                        <td>{{ number_format($info[$i]['sum_fu'], 2, ',', ' ') }}</td>
                        <td>{{ number_format($info[$i+1]['sum_fu'], 2, ',', ' ') }}</td>
                        <td>{{ number_format($info[$i+2]['sum_fu'], 2, ',', ' ') }}</td>
                        <td>{{ number_format($fu_adm, 2, ',', ' ') }}</td>
                        <td><font color="{{ $color }}">{{ number_format($info[$i]['sum_cb'] + $info[$i+1]['sum_cb'] + $info[$i+2]['sum_cb'] + $cb_adm, 2, ',', ' ') }}</td>
                        ...
                        <td><font color="{{ $color }}">{{ number_format($cb_adm, 2, ',', ' ') }}</td>
                    </tr>
                @endif
        ...
        <tr>
            <th>Итог</th>
            <td></td>
            <td><b>{{ number_format($fu_sovet + $fu_kso + $fu_glava + $fu, 2, ',', ' ') }}<b></td>
            <td><b>{{ number_format($fu_sovet, 2, ',', ' ') }}</b></td> 
            <td><b>{{ number_format($fu_kso, 2, ',', ' ') }}</b></td>  
            <td><b>{{ number_format($fu_glava, 2, ',', ' ') }}</b></td> 
            <td><b>{{ number_format($fu, 2, ',', ' ') }}</b></td> 
            <td><font color="{{ $color }}"><b>{{ number_format($cb_sovet + $cb_kso + $cb_glava + $cb, 2, ',', ' ') }}<b></td>
            <td><font color="{{ $color }}"><b>{{ number_format($cb_sovet, 2, ',', ' ') }}</b></td> 
            <td><font color="{{ $color }}"><b>{{ number_format($cb_kso, 2, ',', ' ') }}</b></td> 
            <td><font color="{{ $color }}"><b>{{ number_format($cb_glava, 2, ',', ' ') }}</b></td> 
